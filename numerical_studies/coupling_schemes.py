...
def run_cps_simulation(
    left_system, left_solver, right_system, right_solver, t_end, N, **kwargs
):
    if "sc" in kwargs:
        sc = kwargs["sc"]
    else:
    ...
    while right_iter < N:
        for i in range(sc):
            left_system.result[:, left_iter + 1] = np.squeeze(
                left_solver.compute_timestep(
                    dt_sc, t + i * dt_sc, left_system.result[:, left_iter]
                )
            )
            left_iter += 1
        right_system.result[:, right_iter + 1] = np.squeeze(
            right_solver.compute_timestep(dt, t, right_system.result[:, right_iter])
        )
        right_iter += 1
        t += dt
        left_system.other_u[right_iter] = right_system.result[0, right_iter]
        right_system.other_u[left_iter - (sc - 1) : left_iter + 1] = left_system.result[
            0, left_iter - (sc - 1) : left_iter + 1
        ]
...
def run_implicit_cps_simulation(
    ...
    while right_iter < N:
        ...
        while (fp_iter < max_iters) and (res > tol):
            ...
            for i in range(sc):
                left_temp[:, i + 1] = np.squeeze(
                    left_solver.compute_timestep(dt_sc, t + i * dt_sc, left_temp[:, i])
                )
            right_temp = np.squeeze(
                right_solver.compute_timestep(dt, t, right_system.result[:, right_iter])
            )
            res = l2_norm(
                left_temp[:, -1] - left_system.result[:, left_iter + sc]
            ) + l2_norm(right_temp - right_system.result[:, right_iter + 1])
            assert (
                np.sum(np.abs(left_system.result[:, left_iter] - left_temp[:, 0])) == 0
            )
...
def run_strang_simulation(
    ...
    while right_iter < N:
        left_system.result[:, left_iter + 1] = np.squeeze(
            left_solver.compute_timestep(dt / 2, t, left_system.result[:, left_iter])
        )
        left_iter += 1
        ...
        left_system.result[:, left_iter + 1] = np.squeeze(
            left_solver.compute_timestep(
                dt / 2, t + dt / 2, left_system.result[:, left_iter]
            )
        )
        left_iter += 1
        t += dt
