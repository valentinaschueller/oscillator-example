...
from utility import max_norm, prepare_plot, plot_error_ref
...
def compute_newmark_error(t_stop, N, coupling_scheme_str: str = ""):
    ...
    num_sol = partitioned_newmark_beta(
        t_stop, N, DiffTimescalesPart, coupling_scheme_str
    )
...
def compute_erk4_error(t_stop, N, coupling_scheme_str: str = ""):
    true_sol = analytical_solution(t_stop, N)
    num_sol = partitioned_erk(t_stop, N, 4, DiffTimescalesPart, coupling_scheme_str)
    return true_sol - num_sol


def compute_erk1_error(t_stop, N, coupling_scheme_str: str = ""):
    true_sol = analytical_solution(t_stop, N)
    num_sol = partitioned_erk(t_stop, N, 1, DiffTimescalesPart, coupling_scheme_str)
    return true_sol - num_sol
...
if __name__ == "__main__":
    ...
    errors_newmark_strang = np.array(
        [max_norm(compute_newmark_error(t_stop, N, "strang")) for N in N_list]
    )
    errors_alpha_strang = np.array(
        [max_norm(compute_alpha_error(t_stop, N, "strang")) for N in N_list]
    )
    errors_erk1_strang = np.array(
        [max_norm(compute_erk1_error(t_stop, N, "strang")) for N in N_list]
    )
    errors_erk4_strang = np.array(
        [max_norm(compute_erk4_error(t_stop, N, "strang")) for N in N_list]
    )
    ...
    ax = beautify_plot(ax)
    plt.savefig(
        "convergence_diff_timescales_partitioned_strang.pdf",
        dpi=300,
        bbox_inches="tight",
    )
