...
if __name__ == '__main__':
    ...
    errors_erk4_css_sc = np.array([max_norm(compute_erk4_error(t_stop, N, "css", sc=2)) for N in N_list])
    errors_erk1_css = np.array([max_norm(compute_erk1_error(t_stop, N, "css", sc=1)) for N in N_list])
    errors_erk1_css_sc = np.array([max_norm(compute_erk1_error(t_stop, N, "css", sc=2)) for N in N_list])
    ...
    ax.plot(dt_list, errors_erk1_css_sc, linestyle="--", marker="+", color="green", label=r"ERK1-SC")
    ax.plot(dt_list, errors_erk4_css, linestyle="none", marker="x", color="darkorchid", label=r"ERK4")
    ax.plot(dt_list, errors_erk4_css_sc, linestyle="--", marker="x", color="darkorchid", label=r"ERK4-SC")

    ax.legend(ncol=2, loc='lower right')
    ax = beautify_plot(ax)
    plt.savefig("convergence_diff_timescales_partitioned_explicit.pdf", dpi=300, bbox_inches='tight')