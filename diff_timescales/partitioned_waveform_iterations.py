...
if __name__ == '__main__':
    ...
    errors_newmark_cps = np.array([max_norm(compute_newmark_error(t_stop, N, "implicit-cps", sc=10, interpolation_order=0)) for N in N_list])
    ...
    # errors_erk4_cps = np.array([max_norm(compute_erk4_error(t_stop, N, "implicit-cps", sc=4, interpolation_order=1)) for N in N_list])
    # errors_erk4_cps_io_0 = np.array([max_norm(compute_erk4_error(t_stop, N, "implicit-cps", sc=4, interpolation_order=0)) for N in N_list])
    ...
    # ax.plot(dt_list, errors_erk4_cps, linestyle="none", marker="x", color="darkorchid", label=r"ERK4-1")
    # ax.plot(dt_list, errors_erk4_cps_io_0, linestyle="none", marker="x", color="darkcyan", label=r"ERK4-0")
    # ax.plot(dt_list, errors_erk4_css, linestyle="none", marker="1", color="olive", label=r"ERK4-CSS")

    ax.legend(ncol=2, loc='lower right')
    ax = beautify_plot(ax)
    plt.savefig("convergence_diff_timescales_partitioned_implicit_waveform.png", dpi=300, bbox_inches='tight')